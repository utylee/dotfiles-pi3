function fzf-file-utylee-widget -d "List files and folders"
	...
	if [ $dir = '.' ]
		set mycommand $FZF_CTRL_T_COMMAND				# 빈 커맨드로 인해 현재 디렉일경우		
	else
		set mycommand $FZF_CTRL_T_COMMAND' . $dir'		# 디렉토리를 입력중인 경우	
	end
	#echo $mycommand
